...
class Day6 : public aoc_2024 {
  protected:
    ...
    tuple<int, int, bool> calcDistinctPositions(bool detect_loop) {
      int  changes = 0;
      int  cycles  = 0;
      bool in_loop = false;
      ...
      auto print_field = [&colors, &planted, this]() -> void {
      ...
      auto update_distinct_path = [this, &colors](const point_t& p1, const point_t& p2) -> int {
        int changes = 0;
    ...
    int brute_force_part2() {
      vector<string> origin = field;
      ...
      for (int j = 0; j < origin.size(); j++) {
        for (int i = 0; i < origin[0].length(); i++) {
          if (origin[j][i] == '.') {
            field                 = origin;
            field[j][i]           = '#';
            auto [_, __, in_loop] = calcDistinctPositions(true);
            if (in_loop)
              brute_force_cycles++;
          }
        }
...
TEST_F(Day6, Part1Example) {
  ...
  auto [changes, cycles, in_loop] = calcDistinctPositions(false);
