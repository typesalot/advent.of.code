...
class Day3 : public aoc_2024 {
  protected:
    ...
    int calcMulls(bool use_enables) {
      int total = 0;
...
TEST_F(Day3, Part1Examples) {
  input = "mul(10,5)";
  SetUp();
  EXPECT_EQ(calcMulls(false), 50);

  input = "xmul(2,4)%&mul[3,7]!@^do_not_mul(5,5)+mul(32,64]then(mul(11,8)mul(8,5))";
  SetUp();
  EXPECT_EQ(calcMulls(false), 161);
}

TEST_F(Day3, Part2Examples) {
  //       ---------xxxxxxx.........
  input = "mul(10,5)don't()mull(2,4)";
  SetUp();
  EXPECT_EQ(calcMulls(true), 50);

  //        --------           xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx++++ -------- --> 2*4 + 8*5 = 48
  input = "xmul(2,4)&mul[3,7]!^don't()_mul(5,5)+mul(32,64](mul(11,8)undo()?mul(8,5))";
  SetUp();
  EXPECT_EQ(calcMulls(true), 48);
}

TEST_F(Day3, Part1) {
  int ans = calcMulls(false);
  EXPECT_EQ(ans, 192767529);
}

TEST_F(Day3, Part2) {
  int ans = calcMulls(true);
